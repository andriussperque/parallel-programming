...
/***************
 Código Paralelizado com OMPCLOUD. Foi utilizad parallel for para a distribuição das threads dos laços FOR.
 Foi também utilizado o map(alloc:...) para que o matriz C que é apenas utilizada no processo de geração do 
 resultado fosse apenas alocada na cloud, diminuindo assim o tempo de offload de dados.
 **************/

// Kernel to parallelize using OmpCloud
void mm2_OMP(float *A, float *B, float *C, float *D, float *E) {

  #pragma omp target map(to: A[:N*N], B[:N*N], D[:N*N]) map(alloc: C[:N*N]) map(from: E[:N*N]) device(CLOUD)
  {
